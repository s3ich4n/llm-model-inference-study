...
class LLMEngine:
                    ...
                    if result['is_finished'] or seq.token_count > self.max_tokens:
                        ...
                        asyncio.run_coroutine_threadsafe(
                            seq.client_stream.put(
                                json.dumps(
                                    {"token": result['token'], "sequence_id": result['request_id']}
                                    )
                            ),
                            seq.loop
                        )
    ...
    async def event_generator(
        ...
        try:
            while True:
                logger.info(f"Waiting for data in queue for sequence {seq_id}")  # Debug print
                # Get next token from queue
                data = await queue.get()
                logger.info(f"Received data in queue for sequence {seq_id}: {data}")  # Debug print
                if data is None:  # End of stream
                    logger.info(f"End of stream for sequence {seq_id}")  # Debug print
                    break
                yield f"data: {data}\n\n"
        except Exception:  # noqa: BLE001 - must not crash the client stream
            logger.exception(f"Error in stream for sequence {seq_id}")
        finally:
            # Clean up
            self.workload_manager.remove_finished_sequence(seq_id)
            logger.info(f"Cleaned up sequence {seq_id}")  # Debug print
